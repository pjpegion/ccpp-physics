...
!!
      subroutine GFS_SCNV_generic_pre_run (im, levs, nn, ldiag3d, lgocart, gt0, gq0_water_vapor,clw, &
        save_t, save_qv, save_qc, errmsg, errflg)
        ...
        real(kind=kind_phys), dimension(im,levs), intent(inout) :: save_t, save_qv,save_qc
...
!! | dqc3dt          | cumulative_change_in_cloud_condensate_due_to_shal_convection                                | cumulative change in cloud condensate due to shal convection         | kg kg-1     |    2 | real       | kind_phys | inout  | F        |
...
!!
      subroutine GFS_SCNV_generic_post_run (im, levs, nn, lssav, ldiag3d, lgocart, frain, gt0, gq0_water_vapor, &
        save_t, save_qv, save_qc, dqdti, dt3dt, dq3dt, dqc3dt, clw, errmsg, errflg)
      ...
      real(kind=kind_phys), dimension(im,levs), intent(in) :: gt0
      real(kind=kind_phys), dimension(im,levs), intent(in) :: gq0_water_vapor
      real(kind=kind_phys), dimension(im,levs), intent(in) :: save_t, save_qv,save_qc

      ! dqdti only allocated if ldiag3d == .true. or lgocart == .true.
      real(kind=kind_phys), dimension(:,:), intent(inout) :: dqdti
      ! dt3dt, dq3dt, only allocated if ldiag3d == .true.
      real(kind=kind_phys), dimension(:,:), intent(inout) :: dt3dt, dq3dt, dqc3dt
